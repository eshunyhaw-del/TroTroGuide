...
//
// Given the ordered landmarks along a trip and the rider's live along-route
// position, decide which landmark to surface next: the nearest one AHEAD of the
// rider, on the current leg if any remain, else the first on a later leg. Kept
// pure so the "which landmark, and how far ahead" maths is unit-testable; the
// stateful fetching/prefetching lives in useLookout.ts.
...
export interface LookoutTarget {
  ...
  legIndex: number;
  /** Distance along that leg's route to the landmark (same space as the rider's
   *  projected alongM — see ride-progress.ts). */
  alongM: number;
...
/**
 * A landmark counts as "ahead" only once the rider is at least this far before
 * it — so the card doesn't cling to a landmark the trotro is already passing,
 * and flips to the next one a touch early (matching the "Passed → next" feel).
 */
export const AHEAD_EPS_M = 30;

/**
 * Pick the next landmark to look out for.
 *
 * `targets` must be sorted by (legIndex, alongM) ascending. `legIndex`/`alongM`
 * are the rider's current position from live progress.
 */
 ...
 * `line` is [lat,lng] vertices, `cum` the cumulative metres at each (both as
 * produced everywhere else in the app). Returns null for a degenerate line.
 */
